...
object Aggregations extends App {
  ...
  // counting all
  moviesDF.select(count("*")) // Count all rows, include nulls
  ...
  // Grouping
  val countByGenreDF = moviesDF.groupBy(col("Major_Genre")) // includes nul
  ...
  totalProfitsDF.show()
  totalProfitsDF.select(sum("Total_Profits")).show()
  ...
  moviesDF.select(
    ...
    stddev("US_Gross").as("US_Gross_STD")
  ).show()
